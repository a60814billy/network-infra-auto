...
def diff_cfg(old_cfg: str, new_cfg: str) -> str:
    ...
    """
    old_cfg_line_by_line = [line for line in old_cfg.splitlines() if not line.startswith('!Time: ')]
    new_cfg_line_by_line = [line for line in new_cfg.splitlines() if not line.startswith('!Time: ')]

    diff = ""
    for line in difflib.unified_diff(old_cfg_line_by_line, new_cfg_line_by_line, lineterm=''):
        line = line.strip()
        if len(line) > 0:
            diff += line + "\n"
    return diff

def napalm_sync_config_from_devices(task: Task, dry_run: Optional[bool] = False) -> Result:
    change = False
    diff = ""
    result = ""
